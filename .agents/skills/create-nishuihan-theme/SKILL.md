...
Produce this six-file package under `src/themes/<theme-id>/assets/`:
...
6. Create the subtle texture. Remember that the application stretches it with `object-fit: cover`;
   it is not a tiled CSS texture.
7. Draw the corner assets directly as restrained SVG paths and basic shapes. Do not use raster
   generation for these vector decorations.
8. Compose the preview offline from the final background, character, and corners. Do not bake UI,
   the profession name, calligraphy, a logo, or a `vertical-mark` into any asset.
9. Remove green-screen sources, rejected candidates, review composites, and other temporary files.
...
- Preserve clean mode: all artwork disappears while the palette and high-opacity functional surfaces
  remain usable.
...
- Registry order, metadata, normalization, and all six asset URLs.
...
- Theme background five artwork layers, single-layer failure, theme switching, and clean mode when
  the existing suite expects per-theme coverage.
